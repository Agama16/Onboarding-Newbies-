...
דוקר הוא הכלי המאפשר הרצה של כל סוג application או שרתים אחרים כמו DBים וכו.. על הקונטיינרים, הם מספקים סביבה מותאמת לכל קונטיינר וכולם מופרדים כך שהם לא מפריעים אחד לשני.
...
בניגוד לVM לדוגמה, ההגדרה והעלאה של דוקר הוא קל ופשוט בדרך כלל, ובדרך כלל גם מהיר מאוד.
בניגוד לVM אין מערכת הפעלה נפרדת, הVM הרבה יותר מבודד מדוקר ובגלל זה גם הרבה יותר מאובטח מדוקר, אין אליו גישה מבחוץ. הדוקר רק נותן סביבת עבודה שמדמה בידוד מהמחשב אבל היא על אותה מערכת הפעלה, והקונטיינרים יכולים לחלוק משאבים.
...
בעיקרון הswarm משמש לproduction כגרסה מתקדמת יותר של docker compose, הוא מאפשר לנהל את הapplication בצורה מתקדמת יותר שבה אפשר לעשות גם scaling, ולשנות קונפיגורציות בלי לעשות ריסטרט להכל. זה מעולה כשבאמת יוצרים פרודקשן מתקדם שיכול להיות עמוס או דינאמי מבחינת הצרכים שלו. 
...
********************שאלות סקילה *************************
1.איך עושים בDIND שהdaemon יעלה? עם run docker:dind? אני לא סגורה על זה, בכל מקרה יש systemctl start docker אף פעם לא התאכזבתי. ויש אפשרות עם service.
...
7.הבדל בין CMD לentrypoint: entrypoint מביא את הפקודה שצריך להריץ וCMD את הארגומנטים הדיפולטיבים שצריך להריץ. entrypoint הוא בלתי ניתן לoverride, כלומר תמיד משתמשים בהם, בעוד שבפקודות CMD אם יש פקודה אחרת שהוזרקה בCLI נגיד אפשר לדרוס אותן.
