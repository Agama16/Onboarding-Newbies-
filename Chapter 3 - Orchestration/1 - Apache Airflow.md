...
- הtasks
- הtask dependencies
- הscheduler שאחראי לתזמן את כל הworkflow ואת הtaskים הוא משבץ לexecutors ברגע שכל הdependencies שלהם הושלמו, הexecutor הוא configuration property של הscheduler ולא רכיב נפרד, אבל אחריי גרסה 2.7 הוא יכול להיות מנותק ממנו. הscheduler עוקב אחריי כל קבצי הDAG ופעם בדקה בודק את כל הparsing כדי לראות אם יש משימה שאפשר להתחיל.
...
- הדאטה בייס של הקבצי DAG (מטא דאטה) , שומר מצבי טאסקים, DAGים ומשתנים.
  ...
  הטריגרר הוא תנליך נפרד שמנטר באופן אסינכרוני את כל האופרטורים הdeffered (על כל אחד כזה יש רקורד של trigger בDB), כשהטריגרר מזהה אירוע מסויים הוא מחזיר שולח סיגנל לSCHEDULER שמחזיר את הטסאק לQUEUE . בקוד של הטריגרר הוא פשוט ממתין ובודק באופן חוזר אם האירוע קרה. 
...
2. מה המעטפת של hook ולמה צריך את שניהם? המעטפת זה אופרטור (httpOperator, SqlOperator) ועוד.. צריך את שניהם כי הhook אחראי על החיבור ואיך מבצעים פעולה ואפשר להשתמש בו על כמה וכמה אופרטורים, האופרטור רק מנהל איך בפועל מבצעים את זה , איך מריצים בטאסק בלי שיצטרך לדעת איך להתחבר לשירות. נשתמש בhook גם כשנרצה לכתוב טאסק שמותאם אישית ואין צורך להשתמש באופרטור מוכן מראש, נשתמש בhooks, הם נותנים יותר גמישות ושליטה מלאה על מה המשימה שלנו יכולה לעשות מאשר טמפלט מלא.
3. מה זה הairflow swagger? כלי שמנגיש את הREST API בצורת UI, מאפשר לשלוח בקשות,, לראות endpoints וכו... וככה אפשר לנהל DAGS. הוא נגיש דרך הUI, לדוגמה :                                       http://airflow-host:port/api/v1/ui/ או שליחת בקשת REST עם /api/v1/dags.
5. מה הפורמט של serialized DAG ? פשוט JSON :(
6. מה הbottleneck המרכזי בairflow? הדאטה בייס, ככל שנוצרים יותר DAGים, משימות, DAGRUNS הDB נהיה יותר ויותר עמוס, מנצל יותר CPU ויכול להאט ביצועים.
7. מה התחליף של subDAG? התחליף המומלץ יותר הוא taskGroups שזה קבוצה לוגית של משימות בUI פשוט , יותר קל לביצוע ולא עושים בעיות כמו deadlock על משאבים.
8. מה זה הairflow.cfg? קובץ הקונפיגורציה של לינוקס, מכיל גרסה, הרשאות, איפה הDAG BUNDLE, סוג הEXECUTOR, איפה הלוגים מאוחסנים, מפתחות סודיים וכו..
9. מה זה dynamic task mapping? כלי שמאפשר ליצור טאסקים בזמן ריצה לפי נתונים קיימים מאשר שהDAG יהיה חייב להכיר את הטאסק מראש. לפני שהמשימה תבוצע, הSCHEDULER יצור N עותקים לכל אינפוט אפשרי שלה.
10. איך עובד הuni-testing על DAG? בעזרת dag.test() מריצים את הDAG , הוא לא מצריך SCHEDULER או EXECUTOR אלא רץ לוקלית.
11. מתי עובדים עם kubarnetes וcelery ביחד? מריצים ביחד את האקסקיוטור של שניהם, ואז אפשר גם להשתמש בסלרי שהוא יותר low latency ואין צורך בהקמת כל הפוד (טוב לטאסקים קצרים), וגם בקוברנטיס שטוב למשימות מורכבות כי הוא מגביל משאבים ו"מבודד" משימות. בדיפולט המשימות נשלחות לסלרי.
12. מה זה team? הteam זה קבוצת יוזרים, DAGים ומשאבים מוגדרת כדי ליצור הפרדה בין סביבות עבודה. הם יכולים לחלוק את אותו SCHEDULER וWORKERS אבל לכל אחד יש סביבות EXECUTION שונות, QUEUES שונים, וקונפיגורציות שונות לעיבוד DAG והם יכולים גם להיות מופרדים במשאבים של WORKERS ופודים בכך שכל אחד יקבל POOLS משלו עם סלוטים מוגדרים מראש. מספר הסלוטים שכל team יקבל הוא לפי הPOOL שהוא משתמש בו.
### Real-World Context
