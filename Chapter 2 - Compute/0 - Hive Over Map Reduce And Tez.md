...
הhive כמנוע עיבוד נקרא hiveQL והוא אחראי על תרגום שאילתות לשלבים לוגים וגם למשימות ממש שיעבדו על מנועים חיצוניים כמו ספארק, tez, mapreduce וכו..
...
בדרך כלל יש reducer אחד ואם יש יותר מאחד אז יוצרים פרטישן (בדרכ hash bucketing, אבל יכול להיות מוגדר עי הלקוח).
...
הtez בעצם רץ מתחת לmapreduce, הוא בעצם mapreduce משוכלל יותר שרץ מהר יותר.
...
9. אופטימיזציות : tez עושה cost based optimization שאומר שהוא עובר על אפשרויות שונות לבצע את המשימה ומחשב להן עלויות ובוחר את הזולה ביותר, נוסף על כך הוא עושה פחות כתיבות לHDFS ובכך חוסך גישות לדיסק . גם tez וגם map reduce שומרים על data locality, כלומר הם מבצעים tasks על הdata nodes שעליהם הנתונים שמורים בHDFS. בtez כל השאילתה רצה כtez job אחד ולכן יש פחות overhead סביב התהליך.
10. הservice?/server של hive : לסרבר הנוכחי של hive קוראים hive server 2, הוא אחראי על התקשורת מול הלקוח והחיבור לdriver.  יש את הjar 
