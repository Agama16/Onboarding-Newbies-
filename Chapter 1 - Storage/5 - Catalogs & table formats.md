...
- הlakehouse : שילוב של הdata warehouse והdata lake, הוא מאפשר אחסון של כמויות גדולות של מידע (גם לא מובנה) וגם מאפשר ניתוח של מידע מובנה ושמירה על אמינות ושלמות הנתונים. המטא-דאטה בו נשמר בצורה מובנה נפרדת ובמתודת ETL, אפשר לתשאל אותו בצורת SQL וגם בניתוח מתקדם של ML. לדוגמה databricks וapache iceberg יכולים לשמש כdata lakehouse.
  ...
  בdata warehouse כל המטא דאטה בין אם זה על האחסון עצמו, על התהליכים או על הנתונים, נשמר בmetadata repository בשליטת המערכת ובתצורת schema-on-write. אי אפשר לגשת למטא דאטה מחוץ לdata warehouse.
...
- הiceberg : משתמש בREST catalog שמקביל לפרוטוקול הthrift שבHMS. הוא מאפשר טרנזקציות ACID, ניהול פרטישנים במקביל, שכתוב טבלאות, time travel ודוחס קבצים קטנים לקובץ אחד.
