...
כששאילתה תבקש יותר משאבים , יכנס לתמונה האופטימיזציה, scaling out , אפשרות לשים שאילתות אחרות בqueue עד שיתפנו משאבים או במקרה הצורך לעשות kill לשאילתה, אם מדובר על שאילתות גדולות בנגיד batch processing אז יופעל FTE. כדי לא לעשות kill ישר יעשו במידת הצורך spilling לדיסק, שבו חלק מהפרטישנים של הטבלה הגדולה ביחד עם שורות של הטבלה הקטנה (בפעולות כמו JOIN ) יעברו לדיסק. זה רק במקרה שהאופציה לspilling מופעלת ומדובר באופרטור שתומך בזה , ובמקרה שיש מספיק מקום בדיסק, אחרת יעשה kill.
...
אפשרי גם לעשות role based access control שבו לכל יוזר יש role ולפיו יש לו הרשאות , ADMIN/תפקידים אחרים שניתן להגדיר. יוזר יכול להיות חלק מקבוצה וניתן להגדיר לו גישה לקטלוג.
זה file-based access control ומגדירים אותו ידנית בקובץ JSON.
...
כדי לא לעשות kill אוטומאטית לשאילתה שצריכה יותר משאבים ממה שיש בזיכרון, לפעמים עושים spilling (במצב של FTE) בו רושמים את המידע בזיכרון לדיסק כדי שיוכלו להמשיך לכתוב לזיכרון. זה קורה באופרטורים כמו JOIN, GROUP BY וכו ורק אם הפעלנו את האופציה לעשות spilling ויש מספיק מקום בדיסק. אם האפשרות לעשות ספילינג לא מאופשרת בקונפיגורציה השאילתה תיכשל, ככה גם אם אין מספיק מקום בדיסק.
